...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (wtx.IsZerocoinSpend() && (fZSpendFromMe || wallet->zrupeeTracker->HasMintTx(hash))) {
            //zRUPEE stake reward
            sub.involvesWatchAddress = false;
            sub.type = TransactionRecord::StakeZRUPEE;
        ...
        BOOST_FOREACH (const CTxOut& txout, wtx.vout) {
            ...
            if (mine) {
                ...
                if (ExtractDestination(txout.scriptPubKey, address) && IsMine(*wallet, address)) {
                    // Received by RUPEEEVOLUTION Address
        ...
        if (fAllFromMeDenom && fAllToMeDenom && nFromMe * nToMe) {
            ...
            if (mapValue["DS"] == "1") {
                ...
                if (ExtractDestination(wtx.vout[0].scriptPubKey, address)) {
                    // Sent to RUPEEEVOLUTION Address
                    sub.address = CBitcoinAddress(address).ToString();
                } else {
...
bool IsZRUPEEType(TransactionRecord::Type type)
{
    switch (type) {
        case TransactionRecord::StakeZRUPEE:
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    else if (type == TransactionRecord::Generated || type == TransactionRecord::StakeMint || type == TransactionRecord::StakeZRUPEE || type == TransactionRecord::MNReward) {
