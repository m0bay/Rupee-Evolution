...
CzRUPEEWallet* zwalletMain = NULL;
...
void Shutdown()
{
    ...
    if (nLocalServices == NODE_BLOOM_LIGHT_ZC) {
        lightWorker.StopLightZrupeeThread();
    }
#ifdef ENABLE_WALLET
    delete pwalletMain;
    pwalletMain = NULL;
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "rupeeevolution.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "rupeeevolutiond.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the RUPEEEVOLUTION and zRUPEE money supply statistics") + " " + _("on startup"));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in RUPEEEVOLUTION/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in RUPEEEVOLUTION/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, rupeeevolution, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in RUPEEEVOLUTION/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all RUPEEEVOLUTION specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));

#ifdef ENABLE_WALLET
    strUsage += HelpMessageGroup(_("Staking options:"));
    strUsage += HelpMessageOpt("-staking=<n>", strprintf(_("Enable staking functionality (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-rupeestake=<n>", strprintf(_("Enable or disable staking functionality for RUPEEEVOLUTION inputs (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zrupeestake=<n>", strprintf(_("Enable or disable staking functionality for zRUPEE inputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-backupzrupee=<n>", strprintf(_("Enable automatic wallet backups triggered after each zRUPEE minting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zrupeebackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zRUPEE backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
...
//    strUsage += "  -anonymizerupeeevolutionamount=<n>     " + strprintf(_("Keep N RUPEEEVOLUTION anonymized (default: %u)"), 0) + "\n";
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2017-%i The Rupee Evolution Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
{
    RenameThread("rupeeevolution-loadblk");
...
/** Sanity checks
 *  Ensure that RUPEEEVOLUTION is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Rupee Evolution Core is shutting down."));
...
#endif
    // Make sure only a single RUPEEEVOLUTION process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Rupee Evolution Core is probably already running."), strDataDir));
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Rupee Evolution version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
...
// ********************************************************* Step 5: Backup wallet and verify wallet database integrity
    ...
    //RUPEEEVOLUTION: Load Accumulator Checkpoints according to network (main/test/regtest)
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //RUPEEEVOLUTION specific: zerocoin and spork DB's
                ...
                // RUPEEEVOLUTION: load previous sessions sporks if we have them.
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    //if (chainActive.Height() > Params().Zerocoin_StartHeight()) {
                        RecalculateZRUPEEMinted();
                        RecalculateZRUPEESpent();
                    //}
                    RecalculateRUPEEEVOLUTIONSupply(1);
                }
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Rupee Evolution Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart Rupee Evolution Core to complete") << "\n";
        ...
        zwalletMain = new CzRUPEEWallet(pwalletMain->strWalletFile);
        ...
        //Inititalize zRUPEEWallet
        uiInterface.InitMessage(_("Syncing zRUPEE wallet..."));

        pwalletMain->InitAutoConvertAddresses();

        bool fEnableZRupxBackups = GetBoolArg("-backupzrupee", true);
        pwalletMain->setZRupxAutoBackups(fEnableZRupxBackups);

        //Load zerocoin mint hashes to memory
        pwalletMain->zrupeeTracker->Init();
...
// XX42 Remove/refactor code below. Until then provide safe defaults
    nAnonymizeRupeeEvolutionAmount = 2;
...
//    }
//
//    nAnonymizeRupeeEvolutionAmount = GetArg("-anonymizerupeeevolutionamount", 0);
//    if (nAnonymizeRupeeEvolutionAmount > 999999) nAnonymizeRupeeEvolutionAmount = 999999;
//    if (nAnonymizeRupeeEvolutionAmount < 2) nAnonymizeRupeeEvolutionAmount = 2;
    ...
    LogPrintf("Anonymize RUPEEEVOLUTION Amount %d\n", nAnonymizeRupeeEvolutionAmount);
    ...
    /* Denominations
       ...
       For example:
       1RUPEEEVOLUTION+1000 == (.1RUPEEEVOLUTION+100)*10
       10RUPEEEVOLUTION+10000 == (1RUPEEEVOLUTION+1000)*10
    */
    ...
    */
    ...
    if (nLocalServices & NODE_BLOOM_LIGHT_ZC) {
        ...
        lightWorker.StartLightZrupeeThread(threadGroup);
    }
