...
BOOST_AUTO_TEST_CASE(zerocoin_spend_test)
{
    ...
    cWallet.zrupeeTracker = unique_ptr<CzRUPEETracker>(new CzRUPEETracker(cWallet.strWalletFile));
    ...
    BOOST_CHECK_MESSAGE(receipt.GetStatus() == ZRUPEE_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
    ...
    BOOST_CHECK_MESSAGE(receipt2.GetStatus() == ZRUPEE_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
