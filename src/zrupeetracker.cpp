...
CzRUPEETracker::CzRUPEETracker(std::string strWalletFile)
{
...
CzRUPEETracker::~CzRUPEETracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzRUPEETracker::Init()
{
...
bool CzRUPEETracker::Archive(CMintMeta& meta)
{
...
bool CzRUPEETracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzRUPEETracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzRUPEETracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzRUPEETracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzRUPEETracker::GetSerialHashes()
{
...
CAmount CzRUPEETracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzRUPEETracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzRUPEETracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzRUPEETracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzRUPEETracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzRUPEETracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzRUPEETracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzRUPEETracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzRUPEETracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzRUPEETracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzRUPEETracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzRUPEETracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzRUPEETracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzRUPEETracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzRUPEETracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzRUPEETracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzRUPEETracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzrupee from DB\n", __func__, listDeterministicDB.size());
    }
