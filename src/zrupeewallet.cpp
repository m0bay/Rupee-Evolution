...
CzRUPEEWallet::CzRUPEEWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zrupee seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZRUPEESeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZRUPEESeed_deprecated()) {
                    LogPrintf("%s: Updated zRUPEE seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zrupee seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zrupee wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzRUPEEWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZRUPEECount(nCountLastUsed);
    else if (!walletdb.ReadZRUPEECount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzRUPEEWallet::Lock()
{
    seedMaster = 0;
}

void CzRUPEEWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzRUPEEWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZRUPEE(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzRUPEEWallet::LoadMintPoolFromDB()
{
...
void CzRUPEEWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzRUPEEWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzRUPEEWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zrupeeTracker->HasPubcoinHash(pMint.first)) {
...
bool CzRUPEEWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZRUPEE(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zrupeeTracker which also adds to database
    pwalletMain->zrupeeTracker->Add(dMint, true);
...
void CzRUPEEWallet::SeedToZRUPEE(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzRUPEEWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzRUPEEWallet::UpdateCount()
{
    nCountLastUsed++;
    CWalletDB walletdb(strWalletFile);
    walletdb.WriteZRUPEECount(nCountLastUsed);
...
void CzRUPEEWallet::GenerateDeterministicZRUPEE(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzRUPEEWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzRUPEEWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
